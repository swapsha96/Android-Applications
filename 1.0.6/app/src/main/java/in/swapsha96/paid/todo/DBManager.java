...
public class DBManager extends SQLiteOpenHelper {
    public DBManager(Context context, String name, SQLiteDatabase.CursorFactory factory, int version) {
        super(context, "todo.db", factory, 6);
    }

    @Override
    public void onCreate(SQLiteDatabase db) {
        db.execSQL("CREATE TABLE IF NOT EXISTS todo_lists (id INTEGER PRIMARY KEY AUTOINCREMENT, list TEXT, priority INTEGER);");
    }
    ...
    public String[] showLists(){
        ...
        Cursor cursor =  db.rawQuery( "SELECT * FROM todo_lists ORDER BY priority DESC;", null);
        cursor.moveToFirst();
